...
---
*Last updated: 2026-05-17 after Phase 4.2 (unify SP send into `Action::send` via `SendDestination` enum) complete — `Action::silent_payment_send` factory + `Spends::finish_with_silent_payment_keys` deleted; SP is now a destination type (`SendDestination::SilentPayment`), not a separate action variant. Single `Action::send(id, destination: impl Into<SendDestination>, amount, memos)` constructor for both regular and SP destinations; all 28 existing `Bytes32`-passing callers compile unchanged via `From<Bytes32> for SendDestination`. SEND-04 re-validated; ACTION-API-01 newly validated. Workspace test suite green at 2421 tests. Phase 5 (Bindings) is next.*
