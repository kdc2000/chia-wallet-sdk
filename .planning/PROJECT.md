...
---
*Last updated: 2026-05-18 after Phase 5 (Bindings — Rust facade + JSON descriptor) complete — `chia-sdk-bindings::silent_payments` (428 lines, 11 types) + `bindings/silent_payments.json` (11 entries) expose the full chip-0057 surface across napi/pyo3/wasm; `SendDestination` opaque-handle class added to `bindings/action_system.json` per D-04 with `Action.send(id, destination, amount, memos)` rewired (the Phase 04.2 SC12 commitment); SC4 bindy-macro static_functions gate verified PASS at the macro source (no fallback needed); descriptor↔facade drift audit returns zero drift across 22 methods; 4 AVA round-trip tests (mainnet + testnet address byte-equality + SendDestination factories + Action.send composition) pass alongside 47 pre-existing tests = 51/51 green. BIND-01 + BIND-02 validated. Workspace cargo build/clippy/fmt/machete all green. Phase 6 (Simulator round-trip + bindings E2E + example) is next.*
