...
- [ ] **SEND-03** — `aggregate_sender_sks(sks)` aggregates synthetic secret keys across all wallet-controlled inputs of a single transaction. Returns a `ScalarField`. Hard-errors (no silent fallback) when partial control is detected — multi-party flows must aggregate at sign time across parties. *(Plan 04-01 closed the free-function aggregator + TV4 byte-pin; the Spends-level multi-party hard-error portion lands in Plan 04-03 — re-checks at 04-03 completion.)*
