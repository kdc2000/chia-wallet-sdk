...
- [ ] **POLISH-02** — The three single-`pub-fn` modules `aggregate.rs` (91 lines), `input_hash.rs` (157 lines), and `one_time.rs` (193 lines) under `crates/chia-sdk-driver/src/silent_payments/` are folded into `protocol.rs`. The three source files are deleted; their contents (one `pub fn` + inline `#[cfg(test)] mod tests {}` each) live inside `protocol.rs`. `wc -l crates/chia-sdk-driver/src/silent_payments/protocol.rs` ≤ 700 (target ~640). All external callsites resolve identically; the chip-0057 driver test suite runs the same test count as pre-phase.
