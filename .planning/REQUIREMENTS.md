...
- [ ] **BRIDGE-05** — `SilentPayments.tweakDataFromBlockSpends(coinSpends: CoinSpend[], additions: Coin[]) -> TweakData` static method added to `SilentPayments` namespace class in `chia-sdk-bindings::silent_payments` + descriptor entry in `bindings/silent_payments.json`. Matches the BIND-02 static-method precedent.
