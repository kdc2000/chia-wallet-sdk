...
- [ ] **BRIDGE-01** — `tweak_data_from_block_spends(coin_spends: &[CoinSpend], additions: &[Coin]) -> Result<TweakData, DriverError>` helper lands in new sibling file `crates/chia-sdk-driver/src/silent_payments/block_tweak_data.rs` behind `chip-0057`. Implements full Pass 2a (same-puzzle-hash bucketing) + Pass 2b (iterative Tarjan SCC over opcode-64 `AssertConcurrentSpend` edges extracted via `chia_sdk_types::run_puzzle`) grouping, per-group `A_sum` aggregation, `compute_input_hash` composition, scalar-multiply tweak_point, CHIP §459 identity-element guard, output pairing. Inline `#[cfg(test)] mod tests {}` covers: single-input round-trip vs simulator-helper byte-equality, multi-input SCC grouping, Pass 2b pollution attack (third-party `AssertConcurrentSpend` pointing at a victim's coin must NOT merge groups), non-standard-puzzle skip, identity-element guard, empty-block edge case.
