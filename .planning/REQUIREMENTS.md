...
- [x] **SEND-04** — Silent-payment send composes with the existing `Spends` action system. Production callers use `spends.apply(&[Action::silent_payment_send(recipient, amount, memos)])` (apply-time records the deterministic pieces) and defer ECDH to the chip-0057 SP branch of `Spends::finish_with_keys` after first registering keys via `spends.with_silent_payment_keys(synthetic_pks, synthetic_sks)`. *(Phase 4 shipped a dedicated `Action::silent_payment_send` + `Spends::finish_with_silent_payment_keys`; Phase 4.2 briefly unified the surface into the generic `Action::send` via a `SendDestination` enum; Phase 9.3 reverted that unification — SP send is again a distinct `chip-0057`-gated `Action::silent_payment_send` action and `Action::send(id, puzzle_hash: Bytes32, amount, memos)` is concrete and non-breaking. The apply+finish closure semantics — record `SilentPaymentPending` at apply, derive at finish via `Spends::finish_with_keys` after `with_silent_payment_keys` — are unchanged.)*
- [x] **ACTION-API-01** — Silent-payment send is a distinct `chip-0057`-gated `Action::SilentPaymentSend(SilentPaymentSendAction)` variant constructed via `Action::silent_payment_send(recipient: SilentPaymentAddress, amount: u64, memos: Memos)`. The generic `Action::send(id: Id, puzzle_hash: Bytes32, amount: u64, memos: Memos)` is concrete (no `impl Into`), so all existing `Action::send(id, ph, ...)` and `Action::send(id, x.into(), ...)` callers compile unchanged — no source break. The dedicated action is structurally XCH-only (no `Id` param). Runtime enforcement: `DriverError::SilentPaymentKeysNotRegistered` fires if an SP send is applied without a prior `Spends::with_silent_payment_keys` call before finish. *(Phase 4.2's `SendDestination` enum + `impl Into<SendDestination>` source-broke consumers — verified against Sage: E0283 ×4 on `.into()` send sites + E0560 on the `SendAction { puzzle_hash }` literal — and was reverted in Phase 9.3; `SendDestination` and the now-unreachable `DriverError::SilentPaymentRequiresXch` are removed.)*
...
- [x] **CLEANUP-04** — The inlined `build_tweak_data` helper in `crates/chia-sdk-driver/src/silent_payments/e2e.rs` is removed; tests call the canonical `chia_sdk_test::silent_payments::tweak_data_from_simulator_block` directly. If Cargo's cyclic-dev-dep type confusion blocks the call, tests relocate to a top-level integration target where the cycle resolves. All 3 e2e tests still pass.
...
### Second-pass polish (Phase 8)

These requirements were added after Phase 7 close-out following a second-pass quality survey (2026-05-20). They tighten 4 residual structural details a careful maintainer would flag in PR review — distinct from the 5 issues Phase 7 fixed. Pure refactor; no behavior change.

- [x] **POLISH-01** — `crates/chia-sdk-driver/src/silent_payments/mod.rs` contains zero `pub use foo::*;` wildcard re-exports for public modules. All public re-exports list their symbols explicitly. The 5 protocol primitives (`compute_shared_secret_from_tweak`, `derive_output_tweak`, `derive_onetime_pk`, `derive_onetime_sk`, `puzzle_hash_for_pk`), 3 send-side compositions (`aggregate_sender_sks`, `compute_input_hash`, `derive_one_time_puzzle_hash`), and 3 wire types (`TweakData`, `OutputMeta`, `DetectedSpCoin`) plus scanner exports (`scan_from_tweaks`, `SilentPaymentScan`, `K_MAX_DEFAULT`) all remain reachable via `chia_sdk_driver::silent_payments::*` paths. `pub(crate) use send_keys::*;` is permitted (crate-private). Enforced by `grep -cE '^pub use [a-z_]+::\*;' crates/chia-sdk-driver/src/silent_payments/mod.rs` returning 0.
- [x] **POLISH-02** — The three single-`pub-fn` modules `aggregate.rs` (91 lines), `input_hash.rs` (157 lines), and `one_time.rs` (193 lines) under `crates/chia-sdk-driver/src/silent_payments/` are folded into `protocol.rs`. The three source files are deleted; their contents (one `pub fn` + inline `#[cfg(test)] mod tests {}` each) live inside `protocol.rs`. `wc -l crates/chia-sdk-driver/src/silent_payments/protocol.rs` ≤ 700 (target ~640). All external callsites resolve identically; the chip-0057 driver test suite runs the same test count as pre-phase.
- [x] **POLISH-03** — The `Box<SilentPaymentAddress>` rationale in `crates/chia-sdk-driver/src/action_system/send_destination.rs` lives in exactly one location: the variant-level rustdoc directly above `Box<SilentPaymentAddress>`. The enum-level `/// Cannot derive Copy because SilentPaymentAddress is Clone-only.` paragraph (currently lines 21-23) is removed. `grep -c 'Cannot derive \`Copy\`' crates/chia-sdk-driver/src/action_system/send_destination.rs` returns 0; `grep -c 'large_enum_variant' crates/chia-sdk-driver/src/action_system/send_destination.rs` returns exactly 1.
- [x] **POLISH-04** — `crates/chia-sdk-driver/src/actions/send.rs` chip-0057 dispatch is a single exhaustive `match` where the `SilentPayment` arm calls `handle_silent_payment_send(...)` and `return`s from inside the arm. The `unreachable!("handled above")` arm is removed. `grep -c 'unreachable!' crates/chia-sdk-driver/src/actions/send.rs` returns 0; `grep -c 'handle_silent_payment_send' crates/chia-sdk-driver/src/actions/send.rs` returns exactly 1.

### Real-block TweakData bridge + Python Relation binding (Phase 9)

These requirements close two downstream Python-consumer-blocking gaps in the v1 CHIP-0057 surface: extracting the simulator-helper's pure logic into a real-block-callable helper, and binding `Relation` to all three target languages so multi-input SP sends can be driven from Python/TypeScript/wasm. Additive only; no API breakage.

- [x] **BRIDGE-01** — `tweak_data_from_block_spends(coin_spends: &[CoinSpend], additions: &[Coin]) -> Result<TweakData, DriverError>` helper lands in new sibling file `crates/chia-sdk-driver/src/silent_payments/block_tweak_data.rs` behind `chip-0057`. Implements full Pass 2a (same-puzzle-hash bucketing) + Pass 2b (iterative Tarjan SCC over opcode-64 `AssertConcurrentSpend` edges extracted via `chia_sdk_types::run_puzzle`) grouping, per-group `A_sum` aggregation, `compute_input_hash` composition, scalar-multiply tweak_point, CHIP §459 identity-element guard, output pairing. Inline `#[cfg(test)] mod tests {}` covers: single-input round-trip vs simulator-helper byte-equality, multi-input SCC grouping, Pass 2b pollution attack (third-party `AssertConcurrentSpend` pointing at a victim's coin must NOT merge groups), non-standard-puzzle skip, identity-element guard, empty-block edge case.
- [x] **BRIDGE-02** — `chia-sdk-test::silent_payments::tweak_data_from_simulator_block` refactored to delegate to `chia_sdk_driver::silent_payments::tweak_data_from_block_spends`. Single canonical implementation. All Phase 6 simulator e2e tests (`test_simulator_e2e_unlabeled`, `test_simulator_e2e_labeled`, `test_simulator_e2e_m0_self_change` in `crates/chia-sdk-driver/tests/silent_payments_e2e.rs`) and the two existing inline tests in `crates/chia-sdk-test/src/silent_payments/tweak_data.rs` continue to pass byte-identically.
- [x] **BRIDGE-03** — `Relation` opaque-handle binding in `chia-sdk-bindings::action_system` with 5 methods (`none`, `assert_concurrent`, `is_none`, `is_assert_concurrent`, `equals`) — mirrors `Id`/`SendDestination` precedents. Descriptor entry in `bindings/action_system.json` (NOT a top-level type-group mapping). Surfaces on napi/pyo3/wasm via descriptor-driven bindy-macro generation. Not chip-0057-gated (Relation is general action-system, not SP-specific).
- [x] **BRIDGE-04** — `Spends.prepare` binding signature extended from `prepare(deltas)` to `prepare(deltas, relation: Option<Relation>)`. `None` defaults to `sdk::Relation::None` preserving current behavior at `crates/chia-sdk-bindings/src/action_system.rs:185`. Descriptor entry in `bindings/action_system.json::Spends.methods.prepare` updated with the new `relation: Option<Relation>` arg. Existing single-input pyo3 E2E (`test_unlabeled_e2e`) passes without source changes to the test.
- [x] **BRIDGE-05** — `SilentPayments.tweakDataFromBlockSpends(coinSpends: CoinSpend[], additions: Coin[]) -> TweakData` static method added to `SilentPayments` namespace class in `chia-sdk-bindings::silent_payments` + descriptor entry in `bindings/silent_payments.json`. Matches the BIND-02 static-method precedent.
- [x] **BRIDGE-06** — Three new cross-binding multi-input round-trip tests (`napi/__test__/silent_payments_multi_input.spec.ts`, `pyo3/tests/test_silent_payments.py::test_multi_input_e2e`, `wasm/__test__/silent_payments_multi_input.spec.ts`) demonstrate: farm 2+ XCH coins → single `Action::send` to one SP address → `Spends.prepare(deltas, Relation.assert_concurrent())` → farm bundle → `SilentPayments.tweakDataFromBlockSpends(spends, additions)` → scan → exactly one DetectedSpCoin. Plus `examples/silent_payment.rs` gains a multi-input section demonstrating the same flow in Rust. Includes binding facade entries for `Simulator.blockSpends(height)` + `Simulator.blockOutputs(height)` in `bindings/simulator.json` so binding tests can construct helper inputs without resorting to synthetic fixtures.

### Fund-safety hardening — synthetic sender keys (Phase 9.2)

These requirements close the silent-fund-loss footgun reported in `ISSUE-silent-payment-synthetic-key-guard.md`: `Spends::with_silent_payment_keys` requires synthetic sender keys but enforced it nowhere, so passing raw wallet keys produced a valid-looking bundle whose output landed at an undetectable, unspendable one-time puzzle hash. A layered defense (compile-time newtype + finish-time runtime guard + ergonomic raw-key bindings) makes the mistake impossible to hit silently.

- [x] **GUARD-02** — `SyntheticSecretKey` / `SyntheticPublicKey` newtypes (in `crates/chia-sdk-driver/src/silent_payments/send_keys.rs`) wrap `chia_bls::{SecretKey, PublicKey}` with `from_raw` (synthesizes via the default-hidden `chia_puzzle_types::DeriveSynthetic` path, byte-identical to `puzzle_hash_for_pk`), a `from_synthetic_unchecked` escape hatch, `public_key`, and inner accessors. `Spends::with_silent_payment_keys` takes `IndexMap<Bytes32, SyntheticPublicKey>` / `IndexMap<Bytes32, SyntheticSecretKey>` so passing a raw key is a Rust compile error (E0308). Storage fields stay plain `chia_bls` types, unwrapped via `into_inner()` at the boundary. Newtypes re-exported from the driver SP module + umbrella prelude. Resolves the long-deferred STATE Q2.
- [x] **GUARD-01** — Finish-time runtime guard in `sp_finish_branch` (`crates/chia-sdk-driver/src/action_system/spends.rs`): per non-ephemeral XCH input keyed by `ph = item.asset.p2_puzzle_hash()`, assert `StandardArgs::curry_tree_hash(registered_pk) == ph` and `registered_sk.public_key() == registered_pk`, returning `Err(DriverError::SilentPaymentKeyNotSynthetic)` before any signing. Fires for single-input too; universal across FFI; backstop for the `from_synthetic_unchecked` path.
- [x] **GUARD-03** — Raw-key binding methods on all three targets (`crates/chia-sdk-bindings/src/action_system.rs`): `with_silent_payment_keys` accepts RAW `PublicKey`/`SecretKey` (the newtype cannot cross FFI), synthesizes via `derive_synthetic()` internally, then drives the Rust core. Updates `bindings/action_system.json` + the `SilentPaymentRegisteredKey` shape. GUARD-01 is the safety net for custom-hidden inputs (fails loud, never silent).
...
| SEND-04 | Phase 4 / Phase 4.2 / Phase 9.3 | Phase 4 dedicated action; 4.2 unified via SendDestination; 9.3 reverted to dedicated chip-0057 Action::silent_payment_send + concrete Action::send |
| ACTION-API-01 | Phase 4.2 / Phase 9.3 | 9.3 reframed: SP send is a distinct chip-0057-gated Action::silent_payment_send; Action::send is concrete Bytes32 (non-breaking). SendDestination + SilentPaymentRequiresXch removed |
...
| POLISH-01 | Phase 8 | Replace `pub use foo::*;` wildcards in `silent_payments/mod.rs` with named re-exports |
| POLISH-02 | Phase 8 | Fold `aggregate.rs` + `input_hash.rs` + `one_time.rs` into `protocol.rs` |
| POLISH-03 | Phase 8 | Delete duplicated `SendDestination` Boxing rationale (keep variant-level only) |
| POLISH-04 | Phase 8 | Restructure `Action::send` chip-0057 dispatch to a single `match`; remove `unreachable!()` |
| BRIDGE-01 | Phase 9 | `tweak_data_from_block_spends` helper in `chia-sdk-driver` with Pass 2a + Pass 2b grouping + inline tests (incl. Pass 2b pollution attack) |
| BRIDGE-02 | Phase 9 | `chia-sdk-test::tweak_data_from_simulator_block` delegates to BRIDGE-01's helper; Phase 6 simulator tests pass byte-identically |
| BRIDGE-03 | Phase 9 | `Relation` opaque-handle binding + descriptor in `bindings/action_system.json` |
| BRIDGE-04 | Phase 9 | `Spends.prepare(deltas, relation: Option<Relation>)` binding signature extension |
| BRIDGE-05 | Phase 9 | `SilentPayments.tweakDataFromBlockSpends` static-method binding + descriptor in `bindings/silent_payments.json` |
| BRIDGE-06 | Phase 9 | Multi-input round-trip tests in napi/pyo3/wasm + `examples/silent_payment.rs` multi-input section + `Simulator.block_spends`/`block_outputs` binding facade entries |
| GUARD-02 | Phase 9.2 | `SyntheticSecretKey`/`SyntheticPublicKey` newtypes; `with_silent_payment_keys` takes newtype maps → raw key is a Rust compile error; `from_raw` mirrors `derive_synthetic` byte-for-byte |
| GUARD-01 | Phase 9.2 | Finish-time runtime guard (`curry_tree_hash(pk) == ph` + `sk.public_key() == pk`) → `DriverError::SilentPaymentKeyNotSynthetic` before signing; universal across FFI |
| GUARD-03 | Phase 9.2 | Raw-key binding methods (napi/pyo3/wasm) synthesize via `derive_synthetic()` internally; GUARD-01 is the safety net |

---
*Last updated: 2026-06-03 after Phase 9.3 — reframed ACTION-API-01 + SEND-04: SP send is a distinct `chip-0057`-gated `Action::silent_payment_send` action and `Action::send` is concrete `Bytes32` (non-breaking); the Phase 4.2 `SendDestination` unification was reverted after it was found to source-break consumers (Sage). Previously: Last updated: 2026-05-29 after Phase 9 plan-phase — adds 6 BRIDGE-* requirements (BRIDGE-01..06) closing two downstream-Python-consumer-blocking gaps: real-block TweakData helper (extract pure logic from simulator helper into `chia-sdk-driver::silent_payments::tweak_data_from_block_spends` with full Pass 2a + Pass 2b SCC grouping) and Python/TS/wasm `Relation` binding + extended `Spends.prepare(deltas, Option<Relation>)` signature. Pure additive (no API breakage); v1 surface stays intact. Total v1 + polish + bridge requirements is 47 across 9 phases. Previously: Last updated: 2026-05-20 after Phase 8 discuss-phase — adds 4 second-pass polish requirements (POLISH-01..04) addressing residual structural nits identified by a post-Phase-7 quality survey: `silent_payments/mod.rs` wildcard exports, three single-`pub-fn` modules to fold into `protocol.rs`, duplicated `SendDestination` Boxing rationale, and `Action::send` chip-0057 dispatch using `unreachable!()`. Pure refactor; no behavior change; same shape as Phase 7. POLISH-05 (scanner.rs test extraction) explicitly dropped per repo convention (same call as Phase 7's CLEANUP-05 drop). Total v1 functional requirements remain 32 (all `[x]`); v1+polish total is 41 across 8 phases (CLEANUP-01..04, CLEANUP-06, POLISH-01..04).*
